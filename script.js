// ==============================================
// SISTEMA DE NORMAS NEV/USP - VERSÃO OTIMIZADA
// ==============================================
// ✅ Filtros dinâmicos de Qualis (gerados a partir dos dados)
// ✅ Guias universais inseridos de forma controlada
...
window.state = {
    ...
    filtroQualis: 'Todos',
    searchTerm: '',
    currentView: 'main',       // 'main', 'detail', 'general'
    currentRevista: null
};
...
/**
 * Debounce para otimizar eventos de digitação
 */
...
// ==============================================
// GERENCIAMENTO DE FILTROS (QUALIS)
// ==============================================

/**
 * Extrai todos os valores únicos de Qualis das revistas e gera os botões de filtro.
 */
function renderQualisFilters() {
    const container = document.getElementById('qualis-filters');
    if (!container) return;

    // Obtém todos os valores de qualis presentes nos dados
    const qualisValues = window.state.revistas
        .map(r => r.qualis)
        .filter((v, i, a) => v && a.indexOf(v) === i) // valores únicos
        .sort((a, b) => {
            // Ordenação natural: A1, A2, A3, A4, B1, B2, ...
...
/**
 * Insere os guias gerais (estrutura e roteiros) na main-view,
 * imediatamente antes do container de resultados.
 */
function insertGuiasGerais() {
    const mainView = document.getElementById('main-view');
    if (!mainView) return;

    // Evita duplicação
    if (document.getElementById('guias-gerais-inserido')) return;

    // Localiza o elemento que antecede os resultados (results-count)
    const resultsCountParent = document.getElementById('results-count')?.parentElement;
    if (!resultsCountParent) return;

    // Insere estrutura geral antes dos filtros? Melhor: antes do cabeçalho de resultados.
    // Vamos inserir ambos antes do results-count-parent
    const estruturaHTML = renderizarGuiaEstruturaGeral();
    const roteirosHTML = renderizarRoteirosUniversais();

    // Cria um container para marcar a inserção
    const marker = document.createElement('span');
    marker.id = 'guias-gerais-inserido';
    marker.style.display = 'none';

    // Insere antes do resultsCountParent
    resultsCountParent.insertAdjacentHTML('beforebegin', estruturaHTML + roteirosHTML);
    resultsCountParent.parentNode?.insertBefore(marker, resultsCountParent);
}
...
window.renderizarGuia = renderizarGuia; // expõe para uso em outros contextos
...
async function loadData() {
    ...
    try {
        ...
        // Renderiza filtros (agora dinâmicos)
        renderQualisFilters();

        // Insere os guias gerais (estrutura e roteiros)
        insertGuiasGerais();
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Sistema NEV/USP iniciado - Versão otimizada');
    loadData();
    setupEventListeners();

    // Torna funções globais disponíveis (algumas já estão no escopo window)
    window.goToHome = function () {
        ...
        // Re-renderiza filtros ativos
        renderQualisFilters(); // Isso já redefine o botão "Todos" como ativo
        renderMainView();
        updateResultsCount();
